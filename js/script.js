document.addEventListener('DOMContentLoaded', () => {
    ...
    async function generateRecipe(recipeName) {
        ...
        // Se não temos API key, solicitar ao usuário
        if (!apiKey) {
            apiKey = prompt("Por favor, insira sua chave de API do Google Gemini para gerar receitas:", "");
            
            // Salvar a API key para uso futuro se o usuário fornecer uma
            if (apiKey) {
                localStorage.setItem('geminiApiKey', apiKey);
            } else {
                // Se o usuário cancelar ou não fornecer uma chave
                modalRecipeContent.innerHTML = '<p class="text-center text-red-500 font-bold p-8">É necessária uma chave de API para gerar receitas. <a href="https://ai.google.dev/" class="text-blue-500 underline" target="_blank">Obtenha uma aqui</a>.</p>';
                return;
            }
        }
    ...
    // Adicionar evento para o botão de mudar a API key
    const changeApiKeyBtn = document.getElementById('change-api-key');
    if (changeApiKeyBtn) {
        changeApiKeyBtn.addEventListener('click', () => {
            const currentApiKey = localStorage.getItem('geminiApiKey') || '';
            const newApiKey = prompt("Insira sua chave de API do Google Gemini:", currentApiKey);
